...
for datalist in datalists:  # loops over each dataset
    ...
    for T in Timestamps:
        ...
        for e in epochs:
            try:
                os.mkdir('modelsgru/T={T}_Datalist={e}'.format(T=str(T), e=datalist))
            except OSError:
                pass

            checkpoint = tf.keras.callbacks.ModelCheckpoint(
                filepath='modelsgru/T={T}_Datalist={e}'.format(T=str(T), e=datalist), save_best_only=True,
                save_weights_only=True
            )
            tensorboard = tf.keras.callbacks.TensorBoard(
                log_dir='logsgru/T={T}_Datalist={e}'.format(T=str(T), e=datalist))
            earlystopping = tf.keras.callbacks.EarlyStopping(min_delta=5, patience=10)
            model = tf.keras.models.Sequential([
                tf.keras.layers.GRU(units=100, input_shape=(T, N_train), return_sequences=True),
                tf.keras.layers.Dropout(0.2),
                tf.keras.layers.GRU(units=300, return_sequences=True),
                tf.keras.layers.Dropout(0.2),
                tf.keras.layers.GRU(units=200, return_sequences=True),
                tf.keras.layers.Dropout(0.2),
                tf.keras.layers.GRU(units=100, return_sequences=False),
            ...
            model.summary()
            try:
                os.mkdir('figuresgru/T={T}_Datalist={e}'.format(T=str(T), e=datalist))
            except OSError:
                pass
            ...
            model.load_weights('modelsgru/T={T}_Datalist={e}'.format(T=str(T), e=datalist))
...
#####################get best metrics per loop and produce training graph###############################################
            ...
            pyplot.savefig('figuresgru/T={T}_Datalist={e}/line_graph.png'.format(T=str(T), e=datalist))
            pyplot.show()
            print(pred)
...
######################calculate RMSEs per engine and plot sole engine predictions#######################################
            for id in range(id_array.max()):
                ...
                pyplot.xlabel('Cycle')
                pyplot.ylabel('RUL')
                pyplot.savefig('figuresgru/T={T}_Datalist={e}/ID={id}.png'.format(T=str(T), e=datalist, id=str(id)))
                pyplot.clf()
##################################filter results by RMSE and plot cycles concatenated###################################
            ...
            pyplot.ylabel('RUL')
            pyplot.savefig('figuresgru/T={T}_Datalist={e}/all.png'.format(T=str(T), e=datalist))
            pyplot.show()
            datacount = 0
            score = 0
#################################calculating score if test set was complete##########################
        ...
        if besttimestep == True:
            ...
            pyplot.ylabel('RMSE')
            pyplot.savefig('figuresgru/BestRMSE_Datalist={e}.png'.format(T=str(T), e=datalist))
            pyplot.show()
...
###########################################create metrics vs Timesteps#######################################
    ...
    pyplot.ylabel('MSE')
    pyplot.savefig('figuresgru/bestlistmse_Datalist={e}.png'.format(T=str(T), e=datalist))
    pyplot.show()
    ...
    pyplot.xlabel('Timestep')
    pyplot.ylabel('MAE')
    pyplot.savefig('figuresgru/bestlistmae_Datalist={e}.png'.format(T=str(T), e=datalist))
    pyplot.show()
    ...
    pyplot.xlabel('Timestep')
    pyplot.ylabel('MAPE')
    pyplot.savefig('figuresgru/bestlistmape_Datalist={e}.png'.format(T=str(T), e=datalist))
    pyplot.show()
######################################create-average RMSE per engine graph##############################
    ...
    pyplot.xlabel('Engine ID')
    pyplot.ylabel('MSE')
    pyplot.savefig('figuresgru/avgRMSE_Datalist={e}.png'.format(T=str(T), e=datalist))
    pyplot.show()
    outerresdict['T={T}_Datalist={e}'.format(T=str(T), e=datalist)] = innerresdict
    print(ID, avgrmse)
pprint.pprint(outerresdict)
pprint.pprint(outerresdict, stream=open("resultgru.txt", 'w+'))
